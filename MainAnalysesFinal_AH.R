...
#---- ExportFig2 ----#
#
#save_nhb(Fig2, "../../Revisions Round 2/Main_Figures/Figure2.pdf", cols = 1,height_cm = 8.8)
#gg#save("Figures/Fig2.pdf", width = 8.8, height = 13, units = "cm", device = cairo_pdf)

rm(DifEduc,DifMigration,LabelVars,tidyDifEduc,tidyDifMigration,tidyglhtEduc,tidyglhtMig)


#----- EstTableITT -----
# This generate table 2 in the final paper.
# 
set.seed(999)
...
ITT.ECSAppCreche <- ITTSimultaneous(Y="AppCrecheNP",
                                    treat="Z",
                                    DB=PostDB,
                                    sided.test ="two.sided",
                                    Correction="Westfall",
...
ITT.UseCreche <- ITTSimultaneous(Y="UseCrecheNP",
                                 treat="Z",
                                 DB=PostDB,
                                 Correction="Westfall",
                                 sided.test ="two.sided",
...
## Extraction of data for dynamic text 

# Extract all results for dynamic text - this should run AFTER all analyses
Table1.Analyses <- list(
  ECSApp = ITT.ECSApp,
  ECSAppCreche = ITT.ECSAppCreche,
  ECSUseYes = ITT.ECSUseYes,
  UseCreche = ITT.UseCreche
)

# Define degrees of freedom for each analysis
Table1.df <- list(
  ECSApp = degrees_freedom(ITT.ECSApp$Estimation,"t",cluster = ~SubSampleStrata),      # Replace with your actual DF values
  ECSAppCreche = degrees_freedom(ITT.ECSAppCreche$Estimation,"t",cluster = ~SubSampleStrata),
  ECSUseYes = degrees_freedom(ITT.ECSUseYes$Estimation,"t",cluster = ~SubSampleStrata),
  UseCreche = degrees_freedom(ITT.UseCreche$Estimation,"t",cluster = ~SubSampleStrata)
)

# Create all formatted results for dynamic text
Table1.Text <- create_results_object(Table1.Analyses, Table1.df)

# Store in global environment for access throughout document
#knitr::knit_global()$Table1.Text <- Table1.Text
...
t1.c.UseCreche     <- ITT.UseCreche$ModelSummary$tidy$estimate[ITT.UseCreche$ModelSummary$tidy$term=="Control mean"]
...
MainResultTable1 =
  modelsummary(list(
    ...
    "Application_Non-profit daycare"=ITT.ECSAppCreche$ModelSummary,
    "Access_Early childcare"=ITT.ECSUseYes$ModelSummary,
    "Access_Non-profit daycare"=ITT.UseCreche$ModelSummary
  ),
  coef_map = cm,
...
# NOW "TABLE 1" is called Table 2 in the main text and so is the export !
#save_as_docx(Table_1,path = "../../Revisions Round 2/Main_Tables/Word/Table2.docx")
...
## !!!!!! Now the balance table is called as Table 1 in the main text, and so is the export.
#save_as_docx(Table_2,path = "../../Revisions Round 2/Main_Tables/Word/Table1.docx")
...
Fig3 <- ggplot(Data.Het.EducMig)+
  ...
  labs(y="Models",
       x="Conditional average outcomes (left) and treatment effects (center and right)",
...
#save_nhb(Fig3, "../../Revisions Round 2/Main_Figures/Figure3.pdf", cols = 2, height_cm = 6 )
...
DataPlot_ITT <- bind_rows(
  Het.ITT.AppCreche.Educ2C$ModelSummary0$tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "Control group", Heterogeneity = "SES", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.AppCreche.Educ2C$Tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "ITT", Heterogeneity = "SES", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.AppCreche.Mig$ModelSummary0$tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "Control group", Heterogeneity = "Migration \nbackground", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.AppCreche.Mig$Tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "ITT", Heterogeneity = "Migration \nbackground", Type = "ITT") %>% filter(term %in% term_levels),
)

# Merge ATTs in one DataFrame with the correct fator levels
DataPlot_ATT <- bind_rows(
  Het.ATT.AppCreche.Educ2C$Tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "ATT", Heterogeneity = "SES", Type = "ATT") %>% filter(term %in% term_levels),
  Het.ATT.AppCreche.Mig$Tidy %>% mutate(Y = "Apply for non-profit daycare", panel = "ATT", Heterogeneity = "Migration \nbackground", Type = "ATT") %>% filter(term %in% term_levels)
)
...
# Merge ITTs in one DataFrame with the correct factor levels
DataPlot_ITT <- bind_rows(
  Het.ITT.UseCreche.Educ2C$ModelSummary0$tidy %>% mutate(Y = "Access non-profit daycare", panel = "Control group", Heterogeneity = "SES", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.UseCreche.Educ2C$Tidy %>% mutate(Y = "Access non-profit daycare", panel = "ITT", Heterogeneity = "SES", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.UseCreche.Mig$ModelSummary0$tidy %>% mutate(Y = "Access non-profit daycare", panel = "Control group", Heterogeneity = "Migration \nbackground", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.UseCreche.Mig$Tidy %>% mutate(Y = "Access non-profit daycare", panel = "ITT", Heterogeneity = "Migration \nbackground", Type = "ITT") %>% filter(term %in% term_levels),
  
)

# Merge ATTs in one DataFrame with the correct factor levels
DataPlot_ATT <- bind_rows(
  Het.ATT.UseCreche.Educ2C$Tidy %>% mutate(Y = "Access non-profit daycare", panel = "ATT", Heterogeneity = "SES", Type = "ATT") %>% filter(term %in% term_levels),
  Het.ATT.UseCreche.Mig$Tidy %>% mutate(Y = "Access non-profit daycare", panel = "ATT", Heterogeneity = "Migration \nbackground", Type = "ATT") %>% filter(term %in% term_levels),
)
...
#save_nhb(Fig4, "../../Revisions Round 2/Main_Figures/Figure4.pdf", cols = 2, height_cm = 6 )
...
#save_nhb(Fig5, "../../Revisions Round 2/Main_Figures/Figure5.pdf", cols = 2, height_cm = 8.7 )
...
#save_nhb(Fig6, "../../Revisions Round 2/Main_Figures/Figure6.pdf", cols = 2, height_cm = 8.7 )


#------- ComputePostDoubleLasso ------------
# Define outcomes and their labels
outcomes <- c("ECSApp", "ECSUseYes", "UseCrecheNP", "AppCrecheNP")
outcome_labels <- c("Early childcare application", "Early childcare access", 
                    "Non-profit daycare access", "Non-profit daycare application")
...
data_plot_lasso <- itt_post_lasso %>% 
  ...
  mutate(
    ...
    Y = factor(Y, levels = c(
      ...
      "Non-profit daycare application",
      "Non-profit daycare access"
    ))
  )
...
Fig9 <- ggplot(data_plot_lasso) +
  ...
  scale_x_discrete(name = "Contrasts") +
  scale_shape("Method") +
  facet_wrap(~Y) +
  labs(y = "Intention-to-treat estimates (pp)")

# Display the plot
Fig9
#"../../Revisions Round 2/Main_Figures/#

#"../../Revisions Round 2/Supplementary_Figures'"
save_nhb(Fig9, "../../Revisions Round 2/Main_Figures/Extended_Figure1.pdf", cols = 2, height_cm = 8.7 )

save_nhb(Fig9, "../../Revisions Round 2/Main_Figures/Extended_Figure1.jpg", cols = 2, height_cm = 8.7 ,format="jpg")
...
LATE.UseCreche   <- LATESimultaneous(Y="UseCrecheNP")
LATE.ECSUseYes   <- LATESimultaneous(Y="ECSUseYes")
LATE.ECSApp      <- LATESimultaneous(Y="ECSApp")
LATE.ECSAppCreche <-LATESimultaneous(Y="AppCrecheNP")
...
List.Table3 <- list(
                   ...
                   "Application_Non-profit daycare"          =LATE.ECSAppCreche$ModelSummary,
                   "Access_Early childcare"       =LATE.ECSUseYes$ModelSummary,
                   "Access_Non-profit daycare"               =LATE.UseCreche$ModelSummary
                  )

Table3.Analyses <- list(
  ECSApp = LATE.ECSApp,
  ECSAppCreche = LATE.ECSAppCreche,
  ECSUseYes = LATE.ECSUseYes,
  UseCreche = LATE.UseCreche
)

# Define degrees of freedom for each analysis
Table3.df <- list(
  ECSApp = degrees_freedom(LATE.ECSApp$Estimation,"t",cluster=~SubSampleStrata),
  ECSAppCreche = degrees_freedom(LATE.ECSAppCreche$Estimation,"t",cluster = ~SubSampleStrata),
  ECSUseYes = degrees_freedom(LATE.ECSUseYes$Estimation,"t",cluster = ~SubSampleStrata),
  UseCreche = degrees_freedom(LATE.UseCreche$Estimation,"t",cluster = ~SubSampleStrata)
)

# Create all formatted results for dynamic text
Table3.Text <- create_results_object(Table3.Analyses, Table3.df)
...
##save_as_docx(Table_3, path="../../Revisions Round 2/Supplementary_Tables/word/Extended_Table2.docx")


#flextable_to_jpg(Table_3, filename="../../Revisions Round 2/Supplementary_Tables/jpg/Extended_Table2.jpg")
...
#save_nhb(Fig7, "../../Revisions Round 2/Supplementary_Figures/pdf/Extended_Figure2.pdf", cols = 2, height_cm = 8 ,format="pdf")
#save_nhb(Fig7, "../../Revisions Round 2/Supplementary_Figures/jpg/Extended_Figure2.jpg", cols = 2, height_cm = 8 ,format="jpg")
...
#save_as_docx(ModelT2C, path="../../Revisions Round 2/Supplementary_Tables/word/Extended_Table3.docx")


#flextable_to_jpg(ModelT2C, filename="../../Revisions Round 2/Supplementary_Tables/jpg/Extended_Table3.jpg")
...
Fig8 <- ggplot(Data.Het.Daycare.EducMig)+
  ...
  scale_shape("Coefficients (points), \n95% CI (error bars) \nand Westfall \nsimultaneous \n95% CI (box)")+
  labs(y="Models",
       x="Conditional average outcomes (left) and treatment effects (centre and right)",
       #     caption = paste("Sources:", SourcesStacked,
       #                     "\nITT: Intention to Treat estimates; ATT: Average Treatment on the Treated estimates.",
       #                     "\nStandard errors are cluster-heteroskedasticity robust adjusted at the block level.",
       #                     "\nPoints indicate point estimates and the error bars indicate pointwise 95% Confidence Interval (CI).",
  ...
  theme(axis.text.y = element_text(size = 6))#+#+ vis_theme
# theme(axis.title.y=element_blank(),
#      axis.text.y=element_blank())
Fig8
...
#save_nhb(Fig8, "../../Revisions Round 2/Supplementary_Figures/pdf/Extended_Figure3.pdf", cols = 2, height_cm = 8 ,format="pdf")
save_nhb(Fig8, "../../Revisions Round 2/Supplementary_Figures/jpg/Extended_Figure3.jpg", cols = 2, height_cm = 8 ,format="jpg")
...
# Define labels
OutcomeLabel <- c("Non-profit daycare application", "Non-profit daycare access")
...
#save_as_docx(Table5, path="../../Revisions Round 2/Main_Figures/Extended_Table4.docx")


#flextable_to_jpg(Table5, filename="../../Revisions Round 2/Main_Figures/Extended_Table4.jpg")
...
Table_7 <- 
  AttritionModel %>% 
  merge_at(j=1,i=c(1:2),part="body")|>   
  merge_at(j=1,i=c(5:6),part="body")|>   
  merge_at(j=1,i=c(9:10),part="body") |>   
  # merge_at(j=1,i=c(13:14),part="body") |>   
  #theme_booktabs()|>
  #  separate_header(split="_",opts = c("center-hspan")) |>   # Separate headers
...
#save_as_docx(Table_7, path="../../Revisions Round 2/Supplementary_Tables/word/Extended_Table1.docx")


#flextable_to_jpg(Table_7, filename="../../Revisions Round 2/Supplementary_Tables/jpg/Extended_Table1.jpg")



#------ EstActivityReduction ------------
...
Table6 <- FigAct %>%   theme_booktabs()|>
 ...
 # merge_at(j=1,i=9:16,part="body")|>
  merge_at(j=2,i=1:2,part="body")|>
  merge_at(j=2,i=5:6,part="body")|>
 ...
 # merge_at(j=2,i=13:16,part="body")|>
  ...
  align(part = "body", align = "center")|>                # center   width(j=1,width=3.5,unit = "cm") %>% 
  hline(8,part="body") %>% 
  bg(i=c(1,5),bg='lightgrey',j=c(3:5))
...
#save_as_docx(Table6, path="../../Revisions Round 2/Supplementary_Tables/word/Extended_Table5.docx")


#flextable_to_jpg(Table6, filename="../../Revisions Round 2/Supplementary_Tables/jpg/Extended_Table5.jpg")
...
Table10 <- 
  modelsummary(list("Non-profit daycare application_Control mean"  = Het.ITT.AppCreche.ActiveSES$ModelSummary0,
                    "Non-profit daycare application_ITT" = Het.ITT.AppCreche.ActiveSES$ModelSummary,
                    "Non-profit daycare application_ATT" = Het.LATE.AppCreche.ActiveSES$ModelSummary,
                    "Non-profit daycare access_Control mean"  = Het.ITT.UseCreche.ActiveSES$ModelSummary0,
                    "Non-profit daycare access_ITT" = Het.ITT.UseCreche.ActiveSES$ModelSummary,
                    "Non-profit daycare access_ATT" = Het.LATE.UseCreche.ActiveSES$ModelSummary),
  ...
  merge_at(j=3,part="header")|>
  merge_at(j=1,part="header")|>
  merge_at(j=1,i=c(1:16),part="body")|>
  # merge_at(j=1,i=c(17:32),part="body")|>
  merge_at(j=2,i=c(1:8),part="body")|>
  merge_at(j=2,i=c(9:16),part="body")|>
  # merge_at(j=2,i=c(17:24),part="body")|>
  # merge_at(j=2,i=c(25:32),part="body")|>
  merge_at(j=3,i=c(1:4),part="body")|>
  merge_at(j=3,i=c(5:8),part="body")|>
  merge_at(j=3,i=c(9:12),part="body")|>
  merge_at(j=3,i=c(13:16),part="body")|>
  # merge_at(j=3,i=16+c(1:4),part="body")|>
  # merge_at(j=3,i=16+c(5:8),part="body")|>
  # merge_at(j=3,i=16+c(9:12),part="body")|>
  # merge_at(j=3,i=16+c(13:16),part="body")|>
  italic(i = c(1),  part = "header") %>% 
  italic(j = c(1),  part = "body") %>% 
  align(part = "header", align = "center")|>                # center
  align(part = "body", align = "center")|>                # center   width(j=1,width=3.5,unit = "cm")|>
...
#save_as_docx(Table10, path="../../Revisions Round 2/Main_Figures/Extended_Table6.docx")


#flextable_to_jpg(Table10, filename="../../Revisions Round 2/Main_Figures/Extended_Table6.jpg")



# #save_as_docx(Table10,path = "Tables/Table_10.docx")
