...
EstDoubleDebiased <- function(Y = "ECSApp",                 # Outcome variable name
 ...
 # }
 # 
  required_packages <- c("dplyr", "glmnet", "fixest", "caret", "stringr", "janitor")
  ...
  #  Z_residual_fe <- residuals(fe_model_z)
    ...
    fe_model_y <- feols(Y_residual ~ 0 | FE_vec, data = fe_data, fixef.rm = "none")
    Y_residual_fe <- residuals(fe_model_y)
    
    fe_data_z <- data.frame(Z_residual, FE_vec)
    fe_model_z <- feols(Z_residual ~ 0 | FE_vec, data = fe_data_z, fixef.rm = "none")
    Z_residual_fe <- residuals(fe_model_z)
...
Adjusted p-value and confidence intervals account for simultaneous inference." 
  ...
  return(list("CompareCoef" = list.Glht,"Compare.Y0"=list.Glht0,   "Summary.Model"=Summary.Model,"Data.Summary"=TheModels))
