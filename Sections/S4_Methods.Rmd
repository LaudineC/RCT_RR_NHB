...
$$
Y_{ibs}=\sum_s\sum_b \gamma_{bs} B_{ibs}\times S_{is}+\beta_sZ_{ibs}+\varepsilon_{ibs}
$$
...
$$
Y_{ibs}=\sum_s\sum_b \gamma_{bs} B_{ibs}\times S_{is}+\beta_sZ_{ibs}+\mathbf{\bar{X}^\prime}\rho_s+\upsilon_{ibs}
$$
And Lin regressions:
$$
Y_{ibs}=\sum_s\sum_b \gamma_{bs} B_{ibs}\times S_{is}+\beta_sZ_{ibs}+\mathbf{\bar{X}^\prime}\rho_s+Z\mathbf{\bar{X}^\prime}\tau_s+\upsilon_{ibs}
$$


With $\mathbf{\bar{X}}$, the matrix of covariates selected by the lasso method and previously centred.
...
We adjust p-values and confidence intervals to account for the Family-wise error rates using the Young-Westfall method implemented by the R package multcomp[@BretzEtAl2010].
