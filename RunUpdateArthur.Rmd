...
Het.ITT.App.UsedECEC <- GroupHeterogeneityFnCTRL(DB = PostDB %>% mutate(
...
Het.ATT.App.UsedECEC <- GroupHeterogeneityFnCTRL(DB = PostDBT2 %>% mutate(
  UsedECEC=ifelse(UsedECEC == "Yes","Used ECS","Never used ECS")),
  Outcome = "ECSApp",
  Heterogeneity = "UsedECEC",
  ITT = FALSE,
...
DataPlot_ITT <- bind_rows(
  ...
  Het.ITT.App.UsedECEC$ModelSummary0$tidy %>% mutate(Y = "Apply for early childcare", panel = "Control group", Heterogeneity = "Used ECS", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.App.UsedECEC$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ITT", Heterogeneity = "Used ECS", Type = "ITT") %>% filter(term %in% term_levels),
    Het.ITT.App.Norms$ModelSummary0$tidy %>% mutate(Y = "Apply for early childcare", panel = "Control group", Heterogeneity = "Descriptive norms", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.App.Norms$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ITT", Heterogeneity = "Descriptive norms", Type = "ITT") %>% filter(term %in% term_levels)
)
...
DataPlot_ATT <- bind_rows(
  Het.ATT.App.Info$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ATT", Heterogeneity = "Level of \nknowledge", Type = "ATT") %>% filter(term %in% term_levels),
   Het.ATT.App.UsedECEC$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ATT", Heterogeneity = "Used ECS", Type = "ATT") %>% filter(term %in% term_levels),
      Het.ATT.App.Norms$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ATT", Heterogeneity = "Descriptive norms", Type = "ATT") %>% filter(term %in% term_levels)
)
...
Het.ITT.Use.UsedECEC <- GroupHeterogeneityFnCTRL(DB = PostDB %>% mutate(
  UsedECEC=ifelse(UsedECEC == "Yes","Used ECS","Never used ECS")),
  Outcome = "ECSUseYes",
  Heterogeneity = "UsedECEC",
  ITT = TRUE,
...
Het.ATT.Use.UsedECEC <- GroupHeterogeneityFnCTRL(DB = PostDBT2 %>% mutate(
  UsedECEC=ifelse(UsedECEC == "Yes","Used ECS","Never used ECS")),
  Outcome = "ECSUseYes",
  Heterogeneity = "UsedECEC",
  ITT = FALSE,
...
DataPlot_ITT <- bind_rows(
  ...
  Het.ITT.Use.Norms$ModelSummary0$tidy %>% mutate(Y = "Access early childcare", panel = "Control group", Heterogeneity = "Descriptive norms", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.Use.Norms$Tidy %>% mutate(Y = "Access early childcare", panel = "ITT", Heterogeneity = "Descriptive norms", Type = "ITT") %>% filter(term %in% term_levels),
Het.ITT.Use.UsedECEC$ModelSummary0$tidy %>% mutate(Y = "Access early childcare", panel = "Control group", Heterogeneity = "Used ECS", Type = "ITT") %>% filter(term %in% term_levels),
Het.ITT.Use.UsedECEC$Tidy %>% mutate(Y = "Access early childcare", panel = "ITT", Heterogeneity = "Used ECS", Type = "ITT") %>% filter(term %in% term_levels)
...
DataPlot_ATT <- bind_rows(
  ...
  Het.ATT.Use.Norms$Tidy %>% mutate(Y = "Access early childcare", panel = "ATT", Heterogeneity = "Descriptive norms", Type = "ATT") %>% filter(term %in% term_levels),
  Het.ATT.Use.UsedECEC$Tidy %>% mutate(Y = "Access early childcare", panel = "ATT", Heterogeneity = "Used ECS", Type = "ATT") %>% filter(term %in% term_levels)
...
#present bias trust et active pour psychological costs
...
Het.ITT.App.PresentOrientated <- GroupHeterogeneityFnCTRL(DB = PostDB %>% mutate(
  PresentOrientated=ifelse(PresentOrientated == 1,"Present orientated","Futur oriented")),
  Outcome = "ECSApp",
...
# Define the factors
term_levels <- c("T2-C")
heterogeneity_levels <- c("Temporal orientation", "Trust","Activity")
panel_levels <- c("Control group", "ITT", "ATT")
...
DataPlot_ITT <- bind_rows(
  ...
  Het.ITT.App.PresentOrientated$ModelSummary0$tidy %>% mutate(Y = "Apply for early childcare", panel = "Control group", Heterogeneity = "Temporal orientation", Type = "ITT") %>% filter(term %in% term_levels),
  Het.ITT.App.PresentOrientated$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ITT", Heterogeneity = "Temporal orientation", Type = "ITT") %>% filter(term %in% term_levels),
...
DataPlot_ATT <- bind_rows(
  Het.ATT.App.TrustCreche1or0$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ATT", Heterogeneity = "Trust", Type = "ATT") %>% filter(term %in% term_levels),
   Het.ATT.App.PresentOrientated$Tidy %>% mutate(Y = "Apply for early childcare", panel = "ATT", Heterogeneity = "Temporal orientation", Type = "ATT") %>% filter(term %in% term_levels),
...
Het.ITT.Use.PresentOrientated$ModelSummary0$tidy %>% mutate(Y = "Access early childcare", panel = "Control group", Heterogeneity = "Temporal orientation", Type = "ITT") %>% filter(term %in% term_levels),
Het.ITT.Use.PresentOrientated$Tidy %>% mutate(Y = "Access early childcare", panel = "ITT", Heterogeneity = "Temporal orientation", Type = "ITT") %>% filter(term %in% term_levels)
...
DataPlot_ATT <- bind_rows(
  ...
  Het.ATT.Use.PresentOrientated$Tidy %>% mutate(Y = "Access early childcare", panel = "ATT", Heterogeneity = "Temporal orientation", Type = "ATT") %>% filter(term %in% term_levels)
