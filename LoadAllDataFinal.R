...
MainDB <- read_csv(here("Data","MainDatabase_final.csv")) %>%   mutate( # this version contains the corrected version of the daycare variables on June 17, 2026
  ...
  AppCrecheNP = ifelse(AppCrecheMuni.x+AppCrecheA>=1,1,0), # here we recode the non-profit daycare variables explicitly
  UseCrecheNP = ifelse( UseCrecheMuni+UseCrecheA>=1,1,0)
...
Outcomes <- c("ECSApp","ECSUseYes","UseCreche","AppCreche","AppAssMat","Suivi_administratif1_0", "AppCrecheNP", "UseCrecheNP")
...
reg_MainDB <- MainDB %>%
  dplyr::select(ResponseId, ECSApp, ECSUseYes, AppCrecheNP, 
                UseCrecheNP, Assignment, StrataWave, Educ2, FrenchYNBaseline, HighLowECECBaseline, 
